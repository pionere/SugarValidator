function validate(fileData, CSS = null, localStorage = {}) {
    ...
    function findIdConflicts() {
        ...
        if (graphit != -1) {
            const stack = [[graphit,0,0]];
            const ap = new Set();
            ap.add(-1);
            const label = passageInfo[graphit].name;
            while (stack.length != 0) {
                const next = stack[stack.length - 1];
                stack.pop();

                const depth = next[2];
                const links = passageInfo[next[0]].links.keys();
                ...
                while (true) {
                    ...
                    addInfo(msg + `-> ` + (key == -1 ? '*' : passageInfo[key].name), label);

                    if (key != -1 && !ap.has(key)) {
                        ap.add(key);
                        next[1] = i;
                        stack.push(next);
                        stack.push([key,0,depth+1]);
                        break;
                    }
                }
            }
    ...
    function indexOfTag(html, name, index) {
        while (true) {
            ...
            if (tagName != name || !html.charAt(nameAt[1]).match(/\s/)) {
                index = nameAt[1];
                continue;
            }
            index--;
    ...
    function addCssEntry(map, entry, userCss) {
        if (!map.has(entry) || userCss) {
            map.set(entry, userCss);
        }
    }
    ...
    function checkCssInScript() {
        ...
        const nonSups = new Set();
        let index = 0;
        while (true) {
            index = indexOfTag(fileData, 'style', index);
        if (index === -1) break;
        const contentIndex = fileData.indexOf('>', index) + 1;
        const endIndex = fileData.indexOf('</style>', index);
        if (endIndex === -1 || contentIndex === 0) {
            addWarning(htmlErrorAt('Unclosed stylesheet at ', fileData, index), 'stylesheet');
            index++;
            continue;
        }
        const style = parseCss(fileData, index, contentIndex, endIndex);
        index = endIndex;
        const userCss = getHTMLAttr(style.header, 'id') == 'twine-user-stylesheet';
        const html = style.content;
        let cursor = 0;
        const cssStack = new Map();
        while (true) {
            var start = html.indexOf('{', cursor);
            if (start == -1) {
                break;
            }
            const end = exprAt(html, start+1, /}/, 0);
            if (end == -1) {
                addHtmlError(errorMsgAt('Unclosed css at ', style, start), style.header);
                break;
            }
            ...
            for (var i = params.length - 1; i >= 0; i--) {
                ...
                if (CSS && !CSS.supports(param) && !nonSups.has(name) && name.charAt(0) != '-') {
                    nonSups.add(name);
                    addWarning([[errorMsgAt(`Unsupported CSS feature '${param}' used at `, style, begin)]], style.header);
                }
                if (userCss && ps.has(name)) {
                    addWarning([[errorMsgAt(`Duplicate '${name}' css-attribute at `, style, begin)]], style.header);
                }
                ps.add(name);
            }
            ...
            for (var i = names.length - 1; i >= 0; i--) {
                ...
                const classes = name.match(/[#.]-?[_a-zA-Z]+[_a-zA-Z0-9-]*/g);
                if (classes) {
                    for (const cls of classes) {
                        addCssEntry(cls[0] == '.' ? cssClasses : cssIds, cls.substring(1), userCss);
                    }
                }
        ...
        for (const passage of passages) {
            ...
            if (tags) {
                ...
                for (const tag of tagList) {
                    const twpcs = ['init', 'widget', 'graphit', 'Twine.audio', 'Twine.video', 'Twine.vtt'];
                    if (cssClasses.has(tag)) {
                        cssClasses.set(tag, 0);
                        continue;
                    }
                    if (twpcs.indexOf(tag) == -1) {
                        msg += ', ' + tag;
                    }
                }
                if (msg != '') {
                    msg = msg.substring(2);
                    addWarning([[`Unrecognized passage-tag(s) '${msg}'.`]], passage.header);
                }
            }
            const html = passage.content;
            ...
            let index = 0;
            while (true) {
                index = indexOfTag(html, tag, index);
                if (index == -1) {
                    break;
                }
                ...
                const header = html.substring(index, end);
                const clz = getHTMLAttr(header, 'class');
                if (clz) {
                    const classes = clz.split(' ');
                    let msg = '';
                    for (const name of classes) {
                        if (cssClasses.has(name)) {
                            cssClasses.set(name, 0);
                            continue;
                        }
                        msg += ', ' + name;
                    }
                    if (msg != '') {
                        msg = msg.substring(2);
                        addWarning([[errorMsgAt(`Unrecognized ${tag}-class(es) '${msg}' referenced at `, passage, index)]], passage.header);
                    }
                }
                const id = getHTMLAttr(header, 'id');
                if (id) {
                    if (cssIds.has(id)) {
                        cssIds.set(id, 0);
                    } else {
                        addWarning([[errorMsgAt(`Unrecognized ${tag}-id referenced at `, passage, index)]], passage.header);
                    }
                }
                index = end;
            }
            }
            {
                ...
                while (true) {
                    ...
                    const end = html.indexOf('@@', index + 3);
                    if (end == -1) {
                        break;
                    }
                    const content = html.substring(index + 3, end);
                    ...
                    if (match) {
                        ...
                        if (cssIds.has(id)) {
                            ...
                            addWarning([[errorMsgAt(`Unrecognized id '${id}' referenced at `, passage, index)]], passage.header);
                        }
                    }
                    ...
                    if (matches) {
                        let msg = '';
                        for (const match of matches) {
                            const classes = match.substring(1, match.length - 1).split('.');
                            for (const name of classes) {
                                if (cssClasses.has(name)) {
                                    cssClasses.set(name, 0);
                                    continue;
                        ...
                        if (msg != '') {
                            msg = msg.substring(2);
                            addWarning([[errorMsgAt(`Unrecognized class(es) '${msg}' referenced at `, passage, index)]], passage.header);
                        }
                    }
                    index = end + 2;
                }
        ...
        }
        {
            let index = 0;
            while (true) {
                index = indexOfTag(fileData, 'script', index);
                if (index == -1) {
                    break;
                }
                const endIndex = fileData.indexOf('</script>', index);
                if (endIndex == -1) {
                    break;
                }
                const end = fileData.indexOf('>', index);
                if (getHTMLAttr(fileData.substring(index, end), 'id') != 'script-sugarcube') {
                    index = endIndex;
                    continue;
                }
                index = endIndex;
                const html = fileData.substring(end + 1, endIndex);
                // getHTMLAttr s...
                const matches = html.match(/[^\s"]*[\s]*=[\s]*"[^"]*"/g);
                if (matches) {
                    for (const match of matches) {
                        ...
                        for (const name of ['class', 'id']) {
                            if (match.startsWith(name)) {
                                value = match.substring(name.length).replace(/[\s]*=[\s]*("[^"]*")/, '$1');
                                if (value[0] == '"') {
        ...
        {
            ['saves-import', 'saves-export', 'saves-clear', 'saves-list'].forEach((name) => cssIds.set(name, 0));
            const twip = ['saves-load-', 'saves-delete-', 'header-body-', 'header-heading-', 'header-desc-', 'setting-desc-', 'setting-control-', 'setting-body-'];
            let msg = '';
            for (const key of cssIds.keys()) {
                if (!cssIds.get(key)) continue;
                let genCode = 0;
                for (const pref of twip) {
            ...
            if (msg != '') {
                msg = msg.substring(2);
                addInfo(`Unused css id(s) '${msg}'.`, 'Statistics');
            }
        }
        {
            ['error-toggle', 'enabled', 'error', 'error-source', 'error-view', 'debug', 'hidden', 'link-external', 'link-visited', 'link-broken', 'link-internal', 'marked', 'verbatim', 'link-image',
             'link-disabled', 'open', 'passage', 'passage-out', 'passage-in', 'ui-close', 'datestamp', 'empty', 'buttons', 'no-transition', 'stowed', 'watch-delete', 'saves'].forEach((name) => cssClasses.set(name, 0));
            let msg = '';
            for (const key of cssClasses.keys()) {
                if (!cssClasses.get(key)) continue;
            ...
            if (msg != '') {
                msg = msg.substring(2);
                addInfo(`Unused css class(es) '${msg}'.`, 'Statistics');
            }
        }
    }
