    ...
    function findIdConflicts() {
        ...
        if (graphit != -1) {
            ...
            while (stack.length != 0) {
                ...
                const depth = next[2];
                if (next[0] < 0 || next[0] > passageInfo.length || !passageInfo[next[0]]) {
                    addInfo(`WTF (${next[0]} : ${next[1]} at ${next[2]})`, label);
                    continue;
                }
            ...
            for (var index = names.length - 1; index >= 0; index--) {
                ...
                if (classes) {
                    for (const cls of classes) {
                        if (cls[0] == '.')
                            cssClasses.add(cls.substring(1));
                        else
                            cssIds.add(cls.substring(1));
                    }
                }
