function validate(fileData, localStorage = {}) {
    ...
    function checkCssInScript() {
        ...
        {
            // const twci = ['ui-dialog-body', 'ui-overlay', 'ui-dialog', 'ui-dialog-titlebar', 'ui-dialog-title', 'ui-dialog-close', 'passages', 'story', 'alert-ok', 'autoload-ok'];
            let msg = '';
            for (const key of cssIds.keys()) {
                if (cssIds.get(key) /*&& twci.indexOf(key) == -1*/) {
                    msg += ', ' + key;
                }
            }
            if (msg != '') {
        ...
        {
            const twcs = ['error-toggle', 'enabled', 'error', 'error-source', 'error-view', 'debug', 'hidden', 'link-external', 'link-visited', 'link-broken', 'link-internal', 'marked', 'verbatim', 'link-image',
                          'link-disabled', 'open', 'passage', 'passage-out', 'passage-in', 'ui-close', 'datestamp', 'empty', 'buttons', 'no-transition', 'stowed', 'watch-delete'];
            let msg = '';
            for (const key of cssClasses.keys()) {
                if (cssClasses.get(key) && twcs.indexOf(key) == -1 && !key.startsWith('macro-')) {
                    msg += ', ' + key;
                }
            }
            if (msg != '') {
