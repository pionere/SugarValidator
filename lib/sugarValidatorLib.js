function validate(fileData, localStorage = {}) {
    ...
    function parsePassage(html, startIndex, contentIndex, endIndex) {
        ...
        while(true) {
            ...
            firstIndex += firstMarkup[0].length;
            const exclusion = recodeHTML(content.substring(firstIndex, closingIndex));
            content = content.substr(0, firstIndex) + exclusion + content.substr(closingIndex);
            cursor = firstIndex + exclusion.length + firstMarkup[1].length;
        }
    ...
    function findDeprecatedInScript() {
        ...
        if (initialIndex !== -1) {
            for (const key of deprecationKeys) {
                ...
                while(true) {
                    cursor = fileData.indexOf(key, cursor);
                    if (cursor === -1) break;
                    if (cursor >= maxIndex) break;
                    if (key[0] !== '.') { // Special case, ignore if this is not stand-alone
                        const precedingChar = fileData.substr(cursor - 1, 1);
