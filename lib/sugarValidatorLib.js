    ...
    function findInvalidSetters(html, passage) {
        for (var index = 0; ; index += 5) {
            ...
            if (cc == '$' || cc == '_') {
                ...
                // TODO: check the limited options? (window, Config, Dialog etc...)
            }
            if (!cc.match(/[a-zA-Z]/)) {
                addError(errorMsgAt(`Invalid name variable at `, html, index), passage);
                continue;
            }
            while (true) {
                cursor++;
            ...
            if (cc == '[') {
                continue; // ignore complex array/map setters (for the moment)
            }
            // skip whitespace
            while (true) {
                if (cc.match(/\s/)) {
            ...
            if (cc == '>') {
                addError(errorMsgAt(`Non setter found at `, html, index), passage);
                continue;
            }
            if (cc == '=') {
                continue; // x = ...
            ...
            if (cc == '+' || cc == '-' || cc == '*' || cc == '/' || cc == '%' || cc == '&' || cc == '|' || cc == '^') {
                cursor++;
                cc = html.charAt(cursor);
                if (cc == '=') continue; // x += ... or x -= ...
                addError(errorMsgAt(`Invalid modifier found at `, html, index), passage);
                continue;
            }
            if (cc == 't') {
                cursor++;
                ...
                if (cc == 'o') {
                    cursor++;
                    cc = html.charAt(cursor);
                    if (cc.match(/\s/)) continue; // x to ...
                }
            }
            addError(errorMsgAt(`Invalid setter found at `,html, index), passage);
        }
    }
        ...
        while (true) {
            ...
            if (qNameAt && qNameAt[0] != qNameAt[1]) {
                ...
                if (html.charAt(end).match(/[\s>]/)) {
                    ...
                    if (allMacros[name]) addError(errorMsgAt(`Widget/macro conflict at `, html, defAt), passage);
                    else if (allWidgets.indexOf(widgetName) === -1) allWidgets.push(widgetName);
                    continue;
                }
            }
