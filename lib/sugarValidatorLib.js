function validate(fileData, localStorage = {}) {
    ...
    function findInvalidSetters(html, passage) {
        for (var index = 0; ; index += 5) {
            ...
            while (true) { // eliminate expressions in quotes and comments
            ...
            expr = expr.replace(/ (isnot|is|lte|gte|gt|lt) /g, ' < '); // use standard comparators
            ...
            var wcos = expr.match(/[.x][ ]+\(/);

            while (true) {
                var next = expr.replace(/([x})\]])\.[ ]*x/g, '$1'); // eliminate member references
                next = next.replace(/,[\s]*([,})\]])/g, '$1'); // merge empty expressions in maps/arrays
            ...
            // expr = expr.replace(/([({\[?:;,]) /g, '$1'); // eliminate spacing around brackets, commas and colons
            ...
            while (true) {
                var next = expr.replace(/(x|x\(\)|x?\(x\)|x?\[\])[ ]*&[ ]*(x|x\(\)|\(x\)|\[\])/g, 'x'); // 'resolve' binary operators
