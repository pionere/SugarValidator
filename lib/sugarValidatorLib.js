function validate(fileData, localStorage = {}) {
    ...
    function matchGTLT(html, passage) {
        ...
        while(true) {
            ...
            if (start === -1 || end === -1) {
                if (start !== -1) {
                    return throwError(errorMsgAt(`Found an opening '<<' without matching '>>' at `, html, start) , passage);
                } else {
                    return throwError(errorMsgAt(`Found a closing '>>' without matching '<<' at `, html, end) , passage);
                }
            }
