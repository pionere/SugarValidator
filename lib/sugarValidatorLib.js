function validate(fileData, localStorage = {}) {
    ...
    function parsePassage(html, startIndex, contentIndex, endIndex) {
        ...
        while (true) {
            ...
            if (end === -1) {
                addWarning([[htmlErrorAt(`Found opening '${markup[0]}' without closing '${markup[1]}' at `, content, start)]], header);
                break;
            }
        ...
        return { header, content, exclusions };
    }

    const attrs = [/[\s]+tags=[\s]*"[^"]*"/, /[\s]+pid=[\s]*"[^"]*"/, /[\s]+name=[\s]*"[^"]*"/];
    function getHTMLAttr(html, idx) {
        const match = html.match(attrs[idx]);
        if (!match || match.length != 1) return null;
        const attr = match[0];
        const start = attr.indexOf('"');
        return attr.substring(start+1, attr.length-1);
    }
    ...
    function htmlErrorAt(prefix, html, pos) {
        const text = html.substring(pos, pos + 80);
        return prefix + text;
    }
    function errorMsgAt(prefix, html, pos) {
        ...
        if (html.exclusions) {
            for (var idx = html.exclusions.length - 1; idx >= 0; idx--) {
                ...
                if (start < 0) break;
                text = text.substring(0, start) + exclusionMarkup[excl.type][2] + excl.text + exclusionMarkup[excl.type][3] + text.substring(start);
            }
    ...
    function findIdConflicts() {
        ...
        for (var i = 0; i < passages.length; i++) {
            ...
            const id = getHTMLAttr(header, 1);
            const name = getHTMLAttr(header, 2);
            if (id == null) {
    ...
    function matchGTLT(html, passage) {
        ...
        while (true) {
            ...
            if (nextEnd < nextStart) {
                if (idx < 0) {
                    return throwError(errorMsgAt(`Found an closing '>>' without matching '<<' at `, passage, nextEnd) , passage);
                }
                stack.pop();
                cursor = nextEnd + 2;
                continue;
            ...
            if (idx >= 0) {
                return throwError(errorMsgAt(`Found an opening '<<' without matching '>>' at `, passage, stack[idx]) , passage);
            }
            break; // done
        }
    }
    ...
    function checkInvalidWidgets(html, passage) {
        // Find our tags
        const tags = getHTMLAttr(passage.header, 0);
        ...
        if (index !== -1) addError(errorMsgAt('Widget without a widget tag at ', passage, index), passage);
    }
        ...
        while (true) {
            ...
            if (qNameAt && qNameAt[0] != qNameAt[1]) {
                ...
                if (html.charAt(end).match(/[\s>]/)) {
                    ...
                    if (sugarMacros.has(widgetName)) addError(errorMsgAt(`Widget/macro '${widgetName}' redefinition at `, passage, defAt), passage);
                    else addSimpleMacro(widgetName, 0);
                    index = end;
                    continue;
                }
            }
            addError(errorMsgAt(`Invalid widget definition at `, passage, defAt), passage);
        }
    }
